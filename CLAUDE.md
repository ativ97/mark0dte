...
- **Test count**: 81 tests as of 2026-05-29 — **55 in `test_engine.py`** (49 + 2 sizing + EST-quarantine + resolve-expired + ask-side pricing + warn-tier) + **26 in `test_positions.py`**. Run BOTH files. Plus `synthetic_replay.py` (P0-2 regime gate: GR-0513/0518/0519 force-exit + GR-0529 hold must all PASS). All must pass before any PR.

...
# Last updated: 2026-06-03 (build: tail-day preview [compute_tail_day_preview + config.AVG_WIN_DAY_REFERENCE], contract-weighted intraday_pl [ClosedPositionDB.contracts + migration], ΔGEX velocity [compute_gex_velocity + _gex_history], proper gamma-flip [_compute_gamma_flip vs-spot, supersedes _compute_zero_gamma], + display fixes [neg-GEX wall wording, all-safe/GREEN gated on HIGH CLOSE recs, regime_transition reworded position-neutral]; +15 tests → 127; see docs/method_analysis_2026-06-03.md. Prior 2026-06-02 build: magnet touch% no-overclaim on already-touched/near-spot levels [compute_magnet_forces + day_high/day_low, touch_basis/already_touched] + CAUTION moat-wording inversion fix; then #2–#6 wasted-signals build [narrative single-source-of-truth, direction/size-aware exit conviction, trend_dominant+regime-transition hysteresis, range_position-on-SPX + STRONG_ENTRY RR-floor + RED-while-profit, zero-gamma flip level] +15; then wired under-used signals into decisions [RSI-50 mean-revert factor in analyze_trade_proposal, FADE-off + gap-rejection penalties in auto_propose, premium-RISING exit nudge] +3 → 112 tests | doc-sync audit: pitfall #21 sizing-wiring + $15k ACCOUNT_SIZE + INFORMATIONAL reframe corrected; structure/test-count/env notes reconciled with .windsurfrules). Prior substantive change 2026-06-01 (live checkpoint + 8 post-session fixes: GEX hysteresis, side-aware GEX, heat exposure, FADE-REGIME read, escalation moat-floor, EXIT-grade, 2 display fixes)
...
- **Test count**: **127 tests** as of 2026-06-03 — **101 in `test_engine.py`** (prior 86 + **7** `TestReportingBugFixes20260603` [gamma-flip invariant, neg-GEX wall wording, all-safe gate, regime wording] + **4** `TestTailDayPreview` + **4** `TestGexVelocity`) + **26 in `test_positions.py`** = **127 total** (2026-06-03). In the sandbox 126 pass + `test_main_imports` errors (no fastapi) — run the full **127** in the `mark` env. Run BOTH files. Plus `synthetic_replay.py` (P0-2 regime gate: GR-0513/0518/0519 force-exit + GR-0529 hold must all PASS). All must pass before any PR.
...
│   ├── engine.py           # Core algo: regime, smart moat, positions, recs (~3900 lines)
...
│   ├── test_engine.py      # Unit tests (68 tests)
│   ├── test_positions.py   # evaluate_positions unit tests (26 tests)
│   └── requirements.txt
├── frontend/
│   └── src/app.jsx         # Single-file React dashboard (Tailwind v4, ~2450 lines)
├── docs/
...
└── readme.md               # Setup guide (Python 3.13 + conda env `mark`)
```
...
9. **Frontend is a single file**: `app.jsx` is the entire React app (~2450 lines). No component decomposition yet.
...
21. **Sizing is per-position-wired but INFORMATIONAL, not enforced (P0-3)**: `engine.calculate_position_risk(contracts, width, credit)` IS now wired into `evaluate_positions` (reads `pos.contracts`; emits `position_risk` + `contracts` on each position dict — surfaced via the `EvaluatedPosition` schema and the sizing %-of-account banner in app.jsx). The `contracts` column exists in `database.py` (with the idempotent `_ensure_schema()` ALTER migration) and `PositionCreate.contracts`. Tiers are now **% of `ACCOUNT_SIZE` ($15,000)**: `MAX_RISK_WARN` = 25% (= $3,750, amber notice) and `MAX_RISK_PER_TRADE` = 50% (= $7,500, reference line only). Sizing is deliberately **INFORMATIONAL** — `config.SIZING_HARD_BLOCK = False` (the tool is a decision GUIDE, not an executor; never hard-blocks / never labels "over limit" beyond the advisory `over_limit`/`warn_limit` flags). Remaining gap: portfolio-level aggregation of $-risk across legs (`calculate_portfolio_heat` now sums `total_max_loss`/`pct_of_account` — pitfall #23 — but there is no cross-position cap enforcement, and none is planned). Today's 20-lot was 85% of account — the #1 live-session risk, now surfaced for human judgment. (NOTE: `calculate_position_risk`'s own docstring still reads "contract-count-blind / P0-3a" — that comment is stale vs the wiring above.)

22. **GEX regime must be hysteresis-stabilized + side-aware (2026-06-01)**: (a) Raw `gex_regime` flips POSITIVE↔NEGATIVE on tiny `net_gex` changes near zero — on 6/1 it flipped ~6× and swung the recommended moat 75→104→58→89→77 in minutes. `engine.stabilize_gex_regime(net_gex, prev, band=GEX_REGIME_BAND)` applies a ±20M deadband + stickiness; wired in `main.py` right after `fetch_gex_data` (module state `_gex_regime_prev`) so smart_moat/P0-2/messages read the stabilized sign. Tune `GEX_REGIME_BAND` in config.py. (b) Positive GEX is mean-reversion *protection* for a position ONLY if the gamma wall sits between spot and the short strike (floor ABOVE a put / ceiling BELOW a call). A short option on the far side of the magnet gets dragged TOWARD its strike — the 6/1 7605 call below the 7617 wall. `mean_reverting` in `evaluate_positions` is now side-aware (`gamma_wall > strike` for puts, `< strike` for calls); else `trend_continuation`. Do NOT revert to the global sign-only gate.

23. **Portfolio heat must factor exposure + per-leg danger, not just balance (2026-06-01)**: `calculate_portfolio_heat` used to read SAFE purely on put/call balance — adding an opposite-side leg flipped DANGER→SAFE even though total $-risk rose and a leg was RED (the 6/1 iron condor). It now also sums max-loss (`total_max_loss`, `pct_of_account`), bumps the level past `MAX_RISK_WARN`/`MAX_RISK_PER_TRADE`, and never reads SAFE while any evaluated leg is RED/at-warning (pass `evaluated_positions=`). `portfolio_heat` is a free `dict` in the schema, so new keys pass through (no Pydantic drop). `_bump_level` only ever escalates.
...
26. **`intraday_pl` was contract-BLIND (2026-06-03)**: `closed_pl`/`open_pl` summed PER-SHARE `realized_pl`/`estimated_pl` without ×contracts×100, so the day total was wrong whenever lot sizes differed (6/3 dashboard showed $0.95 vs the real +$1,550). FIX: dollars = `per_share × contracts × 100`. Added `ClosedPositionDB.contracts` (+ idempotent `closed_positions` migration in `_ensure_schema`) and store `pos.contracts` in `close_position`; `main` weights both legs. **Pre-migration closed rows default contracts=1 → their historical $ is understated; forward-correct only.** `realized_pl`/`estimated_pl` stay PER-SHARE everywhere else — never treat them as dollars.

27. **Zero-gamma flip must be net-GEX-VS-SPOT, not cumulative-by-strike (2026-06-03)**: the old `_compute_zero_gamma` accumulated per-strike signed GEX across strikes — conceptually wrong and sign-inconsistent with `net_gex` (6/3: flip ~6900–7124 while spot ~7560 and net_gex NEGATIVE → spot>flip but net<0). The true flip is the SPOT where NET dealer GEX = 0 (BS gamma depends on spot). FIX: `data_fetcher._compute_gamma_flip(chain, t_years, spot)` recomputes net GEX over a spot grid (from `merged`: strike/IV/OI/sign) and interpolates the zero nearest spot; `fetch_gex_data` uses it + an **invariant guard** — suppress (None) unless `spot>flip ⟺ net_gex>0`. `_compute_zero_gamma` retained only for its unit tests (deprecated; unused in prod). Still display-only — NOT wired to the regime gate (H8 unproven).

28. **Single-source display — don't contradict the position recs (2026-06-03)**: three false-comfort reads fixed in `generate_market_insights`/`_compute_regime_transition`: (a) in NEGATIVE GEX a wall is NOT "protecting you" → "unreliable / no firm magnet support"; (b) "All positions are safe" + a GREEN market light are gated on `any_high_close` (no open HIGH CLOSE rec) — else the headline contradicts its own alerts (6/3); (c) `regime_transition` "improving/degrading" was trend-follower-centric (misleads a premium-seller) — reworded position-neutral. The `direction` enum (IMPROVING/DETERIORATING) is unchanged for compatibility.

29. **New decision-support readouts are FREE-DICT + native-typed (2026-06-03)**: `tail_day_preview` (`engine.compute_tail_day_preview` — book max-loss $ / % of account / "= N good days"; `config.AVG_WIN_DAY_REFERENCE` is informational/tunable, NOT fitted) and `gex_velocity` (`engine.compute_gex_velocity` over `main._gex_history` deque — ΔGEX slope M/min, toward-flip, projected min-to-flip). Both are free-dict `TelemetryResponse` fields (extra keys pass through — pitfall #6) and coerce to native bool/int/float (pitfall #25). Informational only; sizing still never blocks (`SIZING_HARD_BLOCK=False`).

30. **Pre-market NaN crashes JSON serialization (2026-06-10)**: Before market open, Alpaca returns only extended-hours bars. TA indicators (RSI, CHOP, ER, VWAP) can produce NaN on sparse data, and these propagate through `regime_data`, `smart_moat_data`, and `dict`-typed TelemetryResponse fields (which Pydantic doesn't validate internally). `json.dumps()` rejects NaN with `ValueError: Out of range float values are not JSON compliant: nan`. FIX: `NaNSafeJSONResponse` as FastAPI's `default_response_class` — custom JSON encoder recursively converts NaN/Inf to `null` before serialization. All endpoints protected.
...
- **P0-2 + sizing (2026-05-29)**: Full-codebase audit → docs/IMPLEMENTATION_PLAN.md (P0–P3 + L1–L11 live findings) + docs/VALIDATION_PLAN.md (T0/T1/T2 ladder) + docs/MONDAY_PREP.md. Synthetic-replay harness `synthetic_replay.py` reproduced the 5/13 EJECT-hold bug. **P0-2 regime-conditional EJECT** in `evaluate_positions`: forces a non-downgradable exit on a with-trend short in a trend-continuation (non-positive-GEX/surge) regime, holds in a positive-GEX mean-reverting regime — validated GR-0513 (5/13 loss) + GR-0529 (5/29 +$1,255 bounce). **Sizing guardrail (informational)**: `config.MAX_RISK_PER_TRADE` + `engine.calculate_position_risk()` (initially unwired on 5/29; wired into `evaluate_positions` in the 5/29 batch-2 sweep — see pitfall #21). Full live-session log: docs/trading_log_2026-05-29.md.
